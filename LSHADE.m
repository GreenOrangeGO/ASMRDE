function [Result,AdaptFuncValue] = LSHADE(PopSize,SearchDimension,SearchScope,AdaptFunc,LoopCount)
%% 该代码为 L-SHADE
...
% 用法：[Result,AdaptFuncValue] = LSHADE(PopSize,SearchDimension,SearchScope,@AdaptFunc,LoopCount);


% 编 制 人：杨强大，刘鹏
% 编制时间：2021.5.11
% 参考文献：Tanabe Ryoji, FukunagaAlex S.（姓后名前，或者说并未改变期刊所谓姓名的先后次序） Improving the search performance of SHADE using linear population size reduction. 2014 IEEE Congress on Evolutionary Computation (CEC), 1658-1665, July 6-11, 2014, Beijing, China.
...
NFEmax = LoopCount*PopSize_Input;     % 相当于根据算法调用时输入的 LoopCount 和 PopSize 计算总适应度评价次数 NFEmax。这么处理的原因是，L-SHADE 算法采取了
...
%% 迭代寻优
% 以下一句程序的功能：初始化用于记录每代迭代所得最优适应度函数值的向量。需要指出的，对于采取变种群策略的 L-SHADE 算法，这里的代，并不是对应于该算法的代。
...
while nfes < NFEmax
   ...
   % *****下面代码的功能：为每个解选出其所对应的 xr1 和 xr2 ***************************************************
    
    % ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    %  关于两个随机解 (即 xr1 和 xr2) 的选择，JADE 所在文献中明确说的是：这两个随机解不相等，他们跟 xi 也不相等；
    %  但作者所给程序中，本质采取的还是常规的序号不同，而不是它们的值不相同；
    %  以下程序给出了两种实现方法，分别对应于序号不同和值不同，但最终采用的还是作者所给程序中的序号不同。
    % ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        
    % ***** 下面代码的功能：产生序号不同的索引 r 和 rr ************************************************
    ...
    if  sum(tmp) == 0                                          % 这里用 0，意味着这里的 mSF、mSCR 是空集，就是没有一个成功的参数       
        ...
        if M(1,t) == 1000 || (max(SCR) == 0 && sum(tmp) > 1)    % 这里用 0，意味着最大的 SCR 是 0，但并不意味是空集，也就是成功个体对应的CR为0，但是sum(tmp)>1，此时CR= 0，并不能把它存起来，而是置为上限
            Mnew(1,t) = 1000;                                  % 1000 代表原文中的 截止 的意思，这里只是个标志，文中只对MCR有限制，对MF没有限制
        else
            Mnew(1,t) = mSCR;
        end
        Mnew(2,t) = mSF;
